...
def test_connections_legacy_layout_is_restored_exactly():
    assert _git_blob_sha(ROOT / "app/web/templates/connections_legacy_8f7481cb.html") == "1276bcec589dcd3a94b7c34dc11b6dfa2a5abd04"
...
def test_naiveproxy_is_appended_only_after_legacy_connections_page():
    ...
    legacy = (ROOT / "app/web/templates/connections_legacy_8f7481cb.html").read_text(encoding="utf-8")
    main = (ROOT / "app/main.py").read_text(encoding="utf-8")

    assert '{% extends "connections_legacy_8f7481cb.html" %}' in wrapper
    assert "{{ super() }}" in wrapper
    assert wrapper.index("{{ super() }}", wrapper.index("{% block content %}")) < wrapper.index('_naiveproxy_panel.html')
