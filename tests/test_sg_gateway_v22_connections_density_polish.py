...
def test_xmux_outer_card_uses_same_horizontal_gutter_as_xray_inner_surfaces():
    css = _css()
    assert "body.page-connections #xray-xmux.xmux1-wrap" in css
    assert "padding-inline: var(--sgui-card-padding);" in css
...
def test_mihomo_internal_vertical_rhythm_is_compact_and_explicit():
    ...
    assert "row-gap: var(--sgui-grid-gap);" in css
