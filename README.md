...
- **Подписки для роутеров.** Для каждого устройства доступны отдельный `OpenWrt · HomeProxy SUB` в проверенном Base64-формате и универсальный Router JSON для внешних интеграторов. Старые SUB-ссылки продолжают работать.
