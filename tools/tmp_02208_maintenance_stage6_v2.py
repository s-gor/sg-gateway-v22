...
PAGE_CSS = '''/* SG-Gateway 22.08 Maintenance page internals. Outer rail belongs to sg-ui-layout-v22-08.css. */
[data-sg-ui-page="maintenance"] { --sg-ui-page-gap: 18px; }
.sg-ui-maintenance-head { margin-inline: 0; padding-inline: 0; }
.sg-ui-maintenance-tabs { width: 100%; min-width: 0; margin: 0; }
[data-sg-ui-page="maintenance"] > .sg-ui-section { width: 100%; margin-inline: 0; }
[data-sg-ui-page="maintenance"] .sg-ui-section-head { width: 100%; margin-inline: 0; }
@media (max-width: 760px) {
  .sg-ui-maintenance-tabs { overflow-x: auto; overscroll-behavior-inline: contain; }
}
'''
...
def write_tests() -> None:
    Path("tests/test_sg_gateway_v22_maintenance_contract_02208.py").write_text(CONTRACT_TEST, encoding="utf-8")
    Path("tests/test_sg_gateway_v22_maintenance_geometry_02208.py").write_text(GEOMETRY_TEST, encoding="utf-8")
...
def _remove_top_level_rule(text: str, selector: str) -> str:
    ...
    if count != 1:
        raise RuntimeError(f"expected one top-level rule for {selector}, found {count}")
    return text


def migrate() -> None:
    path = Path("app/web/templates/maintenance.html")
    text = path.read_text(encoding="utf-8")
    start = text.index("{% block head %}")
    end = text.index("{% endblock %}", start) + len("{% endblock %}")
    styles = "{% block page_styles %}\n" + "\n".join(f"  <link rel=\"stylesheet\" href=\"{{{{ static_asset('{asset}') }}}}\">" for asset in (*LEGACY_CSS, "sg-ui-maintenance-v22-08.css")) + "\n{% endblock %}"
    ...
    for old, new in replacements:
        if old not in text:
            raise RuntimeError(f"missing Maintenance marker: {old}")
    ...
    for selector in (".mtv2-page", ".mtv2-heading"):
        legacy = _remove_top_level_rule(legacy, selector)
